...
def main(args):
    ...
    while True:
        ...
        if(idx_num%150==0):
            cv2.imwrite('/home/dell/qiuzx/FIDTM/gen_imgjson/img/'+video_name+'-'+str(idx_num)+'.jpg', frame)

        with torch.no_grad():
            d6 = model(image)

            count, pred_kpoint = counting(d6)
            box_img = generate_bounding_boxes(pred_kpoint, frame,idx_num)
...
def generate_bounding_boxes(kpoint, Img_data, num):
    if(num%150==0):
        '''generate sigma'''
        pts = np.array(list(zip(np.nonzero(kpoint)[1], np.nonzero(kpoint)[0])))
        leafsize = 2048
        
        dit = {"version": "5.0.1","flags": {}}
        shape_list = []
        if pts.shape[0] > 0: # Check if there is a human presents in the frame
            ...
            # "shapes": [],"imagePath": video_name+"-"+str(num)+".jpg","imageData": None,"imageHeight": pt[1],"imageWidth": pt[0]
            dit["shapes"]=shape_list
            dit["imagePath"]=video_name+"-"+str(num)+".jpg"
            dit["imageData"] = None
            dit["imageHeight"]=Img_data.shape[0]
            dit["imageWidth"]=Img_data.shape[1]
            with open("/home/dell/qiuzx/FIDTM/gen_imgjson/json/"+video_name+"-"+str(num)+".json", "w") as f:
                json.dump(dit, f, indent=4)
    return Img_data
